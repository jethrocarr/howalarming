...
class Envisalink:
		...
		for tube in self.beanstalk_tubes_commands:
			...
			if job:
				...
				if job.body == 'sleep':
					...
					self.sendCommand('040', 'keyboard: disarm', '1' + self.code_master)
				elif job.body == 'panic':
					self.sendCommand('060', 'keyboard: panic', '1')
				elif job.body == 'status':
					self.sendCommand('001', 'keyboard: status')
				else:
					self.printNormal('keyboard: unrecognized command = ' + job.body)
	...
	def decodeResponse(self, word):
		...
		if msg:
			...
			self.printNormal('received [' + word + ']: ' + msg)

			response = {'raw': word, 'code': cmd, 'event': msg}
